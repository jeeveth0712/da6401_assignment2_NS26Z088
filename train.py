"""Training script for all three tasks in DA6401 Assignment-2.

You can train any of the three models using --task flag:
    classify  -> VGG11Classifier  (saves as classifier.pth)
    localize  -> VGG11Localizer   (saves as localizer.pth)
    segment   -> VGG11UNet        (saves as unet.pth)

How to run:
    # Task 1 — classification
    python train.py --task classify --data_root /path/to/pets

    # Task 2 — localization, load encoder weights from Task 1
    python train.py --task localize --data_root /path/to/pets \\
                    --pretrained checkpoints/classifier.pth

    # Task 3 — segmentation, freeze first 3 encoder blocks
...
def dice_loss(
    ...
    We use softmax on predictions and one-hot on targets, then compute
    Dice per class and average. Smooth value avoids divide by zero.

    Args:
        pred_logits: [B, C, H, W] raw logits from the model.
        target:      [B, H, W] ground truth class labels (integers).
        num_classes: Number of classes, default 3 for trimap.
        smooth:      Small constant to avoid zero division.

    Returns:
        Scalar loss value between 0 and 1.
    """
    pred = F.softmax(pred_logits, dim=1)  # convert logits to probabilities
    tgt = F.one_hot(target, num_classes).permute(0, 3, 1, 2).float()  # [B, C, H, W]
    inter = (pred * tgt).sum(dim=(2, 3))  # overlap per class [B, C]
    denom = pred.sum(dim=(2, 3)) + tgt.sum(dim=(2, 3))  # total area [B, C]
    dice = (2.0 * inter + smooth) / (denom + smooth)  # dice per class [B, C]
...
def compute_iou_batch(
    ...
    """Per-sample IoU between predicted and target bounding boxes.

    Args:
        pred_boxes:   [B, 4] predicted boxes in (cx, cy, w, h) format.
        target_boxes: [B, 4] ground truth boxes in same format.

    Returns:
        [B] IoU values, each between 0 and 1.
    """

    def to_xyxy(b):
        # convert center format to corner format for area calculation
        return (
            b[:, 0] - b[:, 2] / 2,
            b[:, 1] - b[:, 3] / 2,
            b[:, 0] + b[:, 2] / 2,
...
def compute_dice(
    ...
    """Mean Dice score over a batch — used for validation only, no gradients needed."""
    with torch.no_grad():
        pred = pred_logits.argmax(dim=1)  # [B, H, W] predicted class per pixel
        score = 0.0
...
def apply_freeze(encoder: nn.Module, mode: str) -> None:
    """Freeze encoder weights based on the chosen strategy.

    Args:
        encoder: The VGG11Encoder module to freeze.
        mode:
            'full_freeze' — freeze all encoder parameters.
            'partial'     — freeze only blocks 1-3, keep 4-5 trainable.
            'none'        — don't freeze anything, train everything.
    """
    if mode == "none":
        return
    ...
    if mode == "partial":
        # freeze early blocks — they learn basic edges/textures that transfer well
        for name, p in encoder.named_parameters():
            if name.startswith(
                ("block1", "pool1", "block2", "pool2", "block3", "pool3")
            ):
                p.requires_grad = False
            else:
    ...
    raise ValueError(
        f"Unknown freeze_mode '{mode}'. Use 'full_freeze', 'partial', or 'none'."
    )
...
def save_checkpoint(
    model: nn.Module, path: str, epoch: int, best_metric: float
) -> None:
    # save model state dict along with epoch and best metric for reference
...
def load_encoder_weights(model: nn.Module, checkpoint_path: str) -> None:
    """Load only the encoder part of weights from a saved checkpoint into model.encoder."""
    ckpt = torch.load(checkpoint_path, map_location="cpu")
    state = (
        ckpt["state_dict"] if isinstance(ckpt, dict) and "state_dict" in ckpt else ckpt
    )
    # filter out only encoder keys and strip the 'encoder.' prefix
    encoder_state = {
...
def train_classifier(args: argparse.Namespace, device: torch.device) -> None:
    ...
    # if --no_augment is passed, use simple resize+normalize for training too
    train_transform = get_val_transforms() if args.no_augment else None
    train_ds = OxfordIIITPetDataset(
        args.data_root, split="train", seed=args.seed, transform=train_transform
    )
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for batch in train_dl:
            ...
            labels = (
                torch.tensor(batch["label"]).long().to(device)
                if not isinstance(batch["label"], torch.Tensor)
                else batch["label"].long().to(device)
            )
            optimizer.zero_grad()
            logits = model(imgs)
        ...
        print(
            f"[Classify] Epoch {epoch:3d}/{args.epochs}  "
            f"train_loss={train_loss:.4f}  train_acc={train_acc:.4f}  "
            f"val_loss={val_loss:.4f}  val_acc={val_acc:.4f}  val_F1={val_f1:.4f}"
        )

        # save checkpoint if this is the best F1 so far
        if val_f1 > best_f1:
            best_f1 = val_f1
...
def train_localizer(args: argparse.Namespace, device: torch.device) -> None:
    """Train bounding box regressor using VGG11 encoder — Task 2.

    Loss = MSELoss + IoULoss combined. IoU loss directly optimizes
    what we care about (box overlap), MSE helps with stability early on.
    """
    ...
    # load encoder weights from classifier checkpoint if provided
    if args.pretrained:
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for batch in train_dl:
            imgs = batch["image"].to(device)
            bboxes = batch["bbox"].to(device)  # [B, 4] in pixel space [0, 224]
            optimizer.zero_grad()
            pred = model(imgs)                 # [B, 4] predicted boxes
...
def train_segmentor(args: argparse.Namespace, device: torch.device) -> None:
    ...
    CE loss handles per-pixel classification well, and Dice loss directly
    optimizes the overlap metric. Together they handle class imbalance better
    than CE alone, especially for the small border class in trimaps.
    """
    ...
    # optionally transfer encoder weights from a pretrained classifier or localizer
    if args.pretrained:
        load_encoder_weights(model, args.pretrained)
    apply_freeze(model.encoder, args.freeze_mode)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for batch in train_dl:
            imgs = batch["image"].to(device)
            masks = batch["mask"].to(device)   # [B, H, W] ground truth trimap
            optimizer.zero_grad()
            logits = model(imgs)               # [B, 3, H, W] predicted logits
        ...
        val_px_acc = px_correct / px_total  # pixel-level accuracy
        ...
        print(
            f"[Segment]  Epoch {epoch:3d}/{args.epochs}  "
            f"train_loss={train_loss:.4f}  val_loss={val_loss:.4f}  "
            f"Dice={val_dice:.4f}  PixAcc={val_px_acc:.4f}"
        )

        # save if best Dice so far
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        ...
        required=True,
        help="Which task model to train.",
    )
    p.add_argument(
        "--data_root",
        type=str,
    ...
    p.add_argument(
        ...
        type=str,
        default="checkpoints",
        help="Folder where .pth files will be saved.",
    )
    p.add_argument(
        "--pretrained",
        type=str,
    ...
    p.add_argument(
        ...
        default="none",
        help="Encoder freezing strategy for Tasks 2 and 3.",
    )
    p.add_argument(
        "--no_augment",
        action="store_true",
        help="Skip training augmentation, use resize+normalize only.",
    )
    p.add_argument(
        "--weight_decay",
        type=float,
...
def main() -> None:
    ...
    # init wandb run — name defaults to task + hyperparams if not specified
    wandb.init(
        project=args.wandb_project,
        name=args.wandb_run_name
        or f"{args.task}_dropout{args.dropout_p}_{args.freeze_mode}",
        config=vars(args),
    )
